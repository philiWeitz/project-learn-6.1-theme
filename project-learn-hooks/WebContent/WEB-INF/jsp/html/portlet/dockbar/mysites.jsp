...
<%-- Copied from /html/taglib/init.jsp --%>
...
<%-- Copied from /html/taglib/ui/my_sites/page.jsp, modifications made --%>
<%
					...
					<c:when test='<%= PropsValues.MY_SITES_DISPLAY_STYLE.equals("simple") %>'>
						...
						<c:choose>
							...
							<c:otherwise>
								...
								<c:if test="<%= showPublicSite && (mySite.getPublicLayoutsPageCount() > 0) && mySite.isUser() %>">
								...
								<c:if test="<%= showPrivateSite && (mySite.getPrivateLayoutsPageCount() > 0) && mySite.isUser() %>">
