...
PortletURL searchURL = PortletURLFactoryUtil.create(request, PortletKeys.SEARCH, layout.getPlid(), PortletRequest.RENDER_PHASE);
searchURL.setParameter("struts_action", "/search/search");
searchURL.setWindowState(WindowState.MAXIMIZED);
searchURL.setPortletMode(PortletMode.VIEW);
%>
...
<div class="dockbar" data-namespace="<portlet:namespace />" id="dockbar">
	<div class="dockbar-custom">
					...
					<li class="search">
						<aui:form method="POST" action="<%=searchURL.toString() %>" class="custom-search">
							<input class="aui-field-input aui-field-input-text" 
										        id="_3_keywords" name="_3_keywords" title="Search" 
										        value="<%= LanguageUtil.get(locale, "dockbar-search-value") %>" 
										        onblur="if (this.value == '') { this.value = '<%= LanguageUtil.get(locale, "dockbar-search-value") %>'; }"
										        onfocus="if (this.value == '<%= LanguageUtil.get(locale, "dockbar-search-value") %>') { this.value = ''; }"
										        size="20" type="text" />
						</aui:form>
					</li>
				</c:if>
		...
		<div class="dockbar-item float-right second-item">
			<ul class="aui-toolbar">
				<c:if test="<%= !group.isControlPanel() && LayoutPermissionUtil.contains(permissionChecker, layout, ActionKeys.UPDATE) || (layoutTypePortlet.isCustomizable() && layoutTypePortlet.isCustomizedView() && LayoutPermissionUtil.contains(permissionChecker, layout, ActionKeys.CUSTOMIZE)) %>">
					<li class="toggle-controls" id="<portlet:namespace />toggleControls">
						<a href="javascript:;">
							<liferay-ui:message key="dockbar-edit-mode" />
						</a>
					</li>
				</c:if>
			...
			<ul class="aui-toolbar user-toolbar">
				<c:if test="<%= user.hasMySites() %>">
					<li class="my-sites has-submenu <%= themeDisplay.isImpersonated() ? "impersonating-user has-submenu" : "" %>" id="<portlet:namespace />mySites">
						<a class="menu-button" href="javascript:;">
							<span class="user"><%=user.getFullName() %></span>
						...
						<div class="aui-menu my-sites-menu aui-overlaycontext-hidden" id="<portlet:namespace />mySitesContainer">
							<div class="aui-menu-content">
								<liferay-util:include page="/html/portlet/dockbar/mysites.jsp" useCustomPage="false" />
								<ul class="links">
									<li class="settings-item">
										<a href="<%= themeDisplay.getURLMyAccount().toString() %>"><liferay-ui:message key="dockbar-link-settings" /></a>
									</li>
									<li class="signout-item">
										<a href="<%= themeDisplay.getURLSignOut() %>"><liferay-ui:message key="dockbar-link-signout" /></a>
									</li>
								</ul>
							</div>
						</div>
